(() => {
  ...
  function startRound(round = randomRound()) {
    ...
    els.roundSub.textContent = `Select every non-specialist player you think was on this roster. Submit at the bottom when done.`;
    setMessage("Select your answers. Nothing counts until you hit Submit.");
    render();
  }
  ...
  function submitRound() {
    ...
    if (state.roundMisses >= MAX_MISSES) {
      state.runOver = true;
      setMessage(`Game over. ${state.roundMisses} misses on this team. Final run score: ${formatNumber(state.totalScore)}.`, "bad");
      saveLeaderboard("LOSS");
    } else {
      state.roundsCleared += 1;
      setMessage(`Round scored. +${formatNumber(state.roundScore)} points. ${state.roundMisses}/3 misses. Keep going.`, "good");
    }
    render();
  }
  ...
  function playerButtonHtml(player) {
    ...
    const missed = (state.revealed || state.submitted) && state.missedIds.has(player.id);
  ...
  function revealAnswers() {
    ...
    if (!state.submitted) {
      setMessage("Submit your round first. Then reveal shows what you missed.", "warn");
      return;
    }
    state.revealed = true;
    setMessage("Correct answers are outlined. This does not change your score.", "warn");
    render();
  }
  ...
  function saveLeaderboard(result) {
    ...
    const run = {
      result,
      finalTeam: `${state.round.year} ${team.city} ${team.name}`,
